...
void cCubeMan2::update()
{
	...
	float angle;
	D3DXVECTOR3 tempDir(0, 0, -1);
	angle = acos(D3DXVec3Dot(&m_vDirection, &tempDir) / (D3DXVec3Length(&m_vDirection) * D3DXVec3Length(&tempDir)));
	if (m_vDirection.x > 0)
	...
	{
		m_destID++;
		if (m_destID == (*m_pTraceline).size() - 1)
			m_destID = 0;
	}
...
void cCubeMan2::setGuide(vector<ST_PC_VERTEX>& line)
{
	m_pTraceline = &line;
}
